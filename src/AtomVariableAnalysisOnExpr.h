#ifndef _AtomVariableAnalysisOnExpr_H
#define _AtomVariableAnalysisOnExpr_H
#include <vector>
...
class AtomVariableAnalysisOnExpr: public IntraDataFlowAnalysis{
...
public:
	...
	AtomVariableAnalysisOnExpr(clang::CFG* cfg,context &ctx,Z3Coding &coding,CFGZ3Code* cfgZ3Code ):IntraDataFlowAnalysis(cfg),c(ctx),z3coding(coding){
		this->cfgZ3Code=cfgZ3Code;
		pre_init();
		doAnalysis();
	}
	~AtomVariableAnalysisOnExpr(){}
					...
					for(Property *p:vlsPre->elements){
						...
						if(av->checkScalaVaribale(scalaVar)){
							if(av->hasStep()){
								...
								av=av->clone();
								av->addIndexUpdateStmt(s,z3Stmt);
								ret->add(av);
							#ifdef _DEBUG	
								std::cout<<"gen: "<< av->toString()<<std::endl;
							#endif

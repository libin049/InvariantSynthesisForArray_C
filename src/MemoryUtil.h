...
//			std::cout<<"clourse1 Time:"<<stop-start<<std::endl;
			...
			start = time(NULL);
			closureformulas=closurePhi(clangStmt,closureformulas,stmt,isKillPhi);
			stop=time(NULL);
...
//					std::cout<<"formula: "<<f<<std::endl;
		 ...
		 * @return 
		 */
		vector<expr> * closurePhi(const clang::Stmt * clangStmt,vector<expr> * closureformulas, expr stmt,bool *isKillPhi){
			
			//std::cout<<"closureformulas is: "<<toString(closureformulas)<<std::endl;
			expr primedindex=stmt.arg(0);
			expr primedstep=primedLift(getStepFromIndexUpdate(stmt));
			expr init=getInitFromIndex(clangStmt,z3Coding.unprime(primedindex));
...
//		}
		bool isIndexUpdate(const clang::Stmt* clangStmt, expr stmt){
			...
			for(Property* p: vlsOut->elements){
				...
				if(equal(av->indexUpdateStmtz3coding,stmt)){
					return true;
				}
			}
			return false;
...
//		}
		expr getInitFromIndex(const clang::Stmt *clangStmt,expr index){
			FlowSet* out=ava->mapToStmtOut.at(clangStmt);
			ValueListSet* vlsOut=(ValueListSet*)out;
			for(Property* p: vlsOut->elements){
				AtomVariabla * av=(AtomVariabla *)p;
		...
		vector<expr> * Pos(const clang::Stmt* clangStmt,vector<expr> * Pre, expr stmt, map<std::string,expr> * scopeMemoryUnits){
			...
			vector<expr> * closureformulas=closure(clangStmt,Pre,primedformulas,stmt);
			//std::cout<<"closureformulas is: "<<toString(closureformulas)<<std::endl;
		 ...
		 * @return 
		 */
		vector<expr> * closure(const clang::Stmt* clangStmt,vector<expr> * Pre,vector<expr> * primedformulas, expr stmt){
			vector<expr> * closureformulas=new vector<expr>();
			if(isAssigndToScaleVariable(stmt)||!isAssignment(stmt)){
				if(isIndexUpdate(clangStmt,stmt)){
					...
					closureformulas=closureIndexUpdate(clangStmt,Pre,primedformulas,stmt,&isKillPhi);
